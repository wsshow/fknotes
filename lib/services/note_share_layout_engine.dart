...
class NoteShareLayoutEngine {
  ...
  double _blockHeight(
    ...
    if (block.type == NoteBlockType.divider) return 21 * scale;
    if (block.type == NoteBlockType.attachment) {
      ...
      if (image && options.includeImages) {
        return (landscape ? 92 : 150) * scale;
      }
      return 58 * scale;
    }
    final style = _textStyle(block, scale);
    ...
    final text = block.type == NoteBlockType.rawMarkdown
        ? MarkdownText.toPlainText(block.text)
        : block.text;
    final textHeight = _measureText(
      text,
      style,
      math.max(40, width - prefixWidth),
      textDirection,
    );
    final decoration = switch (block.type) {
      NoteBlockType.quote => 14.0,
      NoteBlockType.code || NoteBlockType.rawMarkdown => 20.0,
      _ => 0.0,
    };
    ...
    return textHeight + (decoration + 9) * scale;
  }
  ...
  double _minimumSplitHeight(
    ...
    return lineHeight + (decoration + 9) * scale;
  }

  TextStyle _textStyle(NoteBlockData block, double scale) {
