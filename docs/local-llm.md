...
- 生成结果必须先预览，只有用户确认后才会作为一次可撤销编辑写入笔记。

当前产品能力包括总结笔记、提取待办和润色内容。统一接口已保留多轮消息、能力声明、后端选择、流式事件、取消、指标和结束原因，可继续承载聊天、选中文本处理及图片理解等功能。
...
2. 首次加载后分别运行总结、待办和润色，记录首 token 时间与生成速度。
