...
- 可选 MNN 本地语言模型，支持对当前笔记进行总结、提取待办和润色；结果先流式预览，确认后才写入笔记
...
- MNN 3.6：Android / iOS arm64 设备端大语言模型推理
...
- Android 申请相机、录音和网络权限。网络仅在用户明确点击后，从 ModelScope 下载可选的离线语音模型；笔记、附件、OCR 和转写内容不会上传。
